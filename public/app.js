...
function showPage(p) {
  ...
  if (window.location.pathname === dest || (p === 'home' && window.location.pathname === '/')) {
...
function toggleMenu(){
  ...
  if(!m){
    ...
    m.innerHTML=`
      <a href="/">Home</a>
      <a href="/diet-goals">Diet Goals</a>
      <a href="/diet-plans">Diet Plans</a>
      <a href="/about">About Us</a>
      <a href="/forge-score" style="color:var(--cyan)">⚡ Forge Score</a>
    `;
...
function renderForgeScore() {
  if (!document.getElementById('gaugeNum')) return;
  if (!lastCalcData) {
...
function logWeek() {
  ...
  if (!w || !e || Number.isNaN(s)) { alert('Please fill in weight, energy, and sessions.'); return; }
...
function drawWeightChart(entries) {
  ...
  if (!ctx) return;
  if (entries.length < 2) { ctx.clearRect(0,0,canvas.width,canvas.height); if(empty)empty.style.display = 'block'; return; }
